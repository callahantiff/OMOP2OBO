...
@click.option('--umls_data', type=click.Path(exists=True), required=True)
@click.option('--omop_data', type=click.Path(exists=True), required=True, default=None)
...
def main(ont_file: str, tfidf_mapping: str, domain: str, onts: list, omop_data: str, primary_key: str,
    ...
    (OMOP) clinical terminology concepts to Open Biomedical Ontology (OBO) terms. Successfully running this program
    requires several input parameters, which are specified below:

    PARAMETERS:
        ont_file: 'resources/ontology_source_list.txt'
        tfidf_mapping: "yes" if wanting to perform cosine similarity mapping using a TF-IDF matrix.
        domain: clinical domain of input data (i.e. "conditions", "drugs", or "measurements").
        onts: A comma-separated list of ontology prefixes that matches 'resources/ontology_source_list.txt'.
        omop_data: The filepath to the clinical data needing mapping.
    ...
    NOTE: Several dependencies must be addressed before running this file. Please see the README for instructions.
    ...
    ont_data = ont_explorer.master_ontology_dictionary


    ###############################################
    # PROCESS CLINICAL DATA AND GENERATE MAPPINGS #
    ###############################################
    # # STEP 1: Process clinical vocabularies
    # omop_data = ClinicalDataProcessor(umls_data, omop_data)
    ...
    mapper = ConceptAnnotator(clinical_file=omop_data,
    ...
    mappings.to_csv(outfile + domain.upper() + date_today + '.csv', sep=',', index=False, header=True)
    ...
    if tfidf_mapping is not None:
        sim = SimilarStringFinder(clinical_file=omop_data,
                                  ontology_dictionary={k: v for k, v in ont_data.items() if k in onts},
                                  primary_key=primary_key,
                                  concept_strings=concept_strings)
    ...
    updated_maps.to_csv(outfile + domain.upper() + date_today + '.csv', sep=',', index=False, header=True)
